...
public class SavedRouteStoreTest {
    ...
    public void save_persistsDestinationStopsAndNewestFirstOrder() {
        SavedRouteStore store = new SavedRouteStore(preferences);
        ...
        List<SavedRoute> routes = new SavedRouteStore(preferences).list();
    ...
    public void renameAndRemove_updateStoredRoutes() {
        ...
        List<SavedRoute> renamedRoutes = new SavedRouteStore(preferences).list();
        assertEquals("Updated", renamedRoutes.get(0).name);

        store.remove(route.id);

        assertTrue(new SavedRouteStore(preferences).list().isEmpty());
    }
}
