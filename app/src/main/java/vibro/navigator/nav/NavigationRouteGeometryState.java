...
final class NavigationRouteGeometryState {
    ...
    void loadRoute(@NonNull GeoJsonRoute newRoute) {
        route = newRoute;
    ...
    void rememberSegment(@NonNull PolylineIndex.Match match) {
        lastSegmentIndex = match.segmentIndex;
    }

    @Nullable
    PolylineIndex.Match match(@NonNull Location location) {
    ...
    double expectedBearingDegrees(@NonNull PolylineIndex.Match match) {
        return NavigationExpectedBearingResolver.resolve(polylineIndex, match);
    }

    boolean isWithinDestinationReachedRadius(@NonNull Location location, float accuracyMeters) {
