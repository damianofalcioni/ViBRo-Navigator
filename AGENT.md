...
- `NavState` is the immutable navigation display snapshot. Keep route direction/progress line assembly in `NavStateTextFactory`, compass-state assembly in `NavCompassStateFactory`, and primitive user-visible navigation/notification formatting in `NavigationTextFormatter`. Keep GPS status field formatting and validity checks in `NavigationGpsTextFormatter` so turn/off-route copy and GPS telemetry copy remain separate.
