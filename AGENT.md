...
- `NavigationSession` is the session-level coordinator. Keep filtered location, route progress, blocked-road state, turn progression, route display state, and route-request lifecycle split across dedicated state/policy classes instead of collapsing them back together. Keep the session handoff contracts as package-level value types: `NavigationLocationUpdateResult`, `NavigationRouteRequestSnapshot`, and `NavigationTurnEvent`. Within filtered location handling, keep recent filtered-fix motion history, stationary detection, speed fallback, and movement-derived bearing in `NavigationLocationMotionModel` rather than folding those heuristics into `NavigationSessionLocationState`. Rolling route-progress samples for smoothed accuracy, ETA speed, and direction-of-progress evidence belong in `NavigationRouteProgressTracker`. Heading-source selection for display state belongs in `NavigationSessionHeadingResolver`.
- `NavigationSessionRouteState` owns active-route evaluation: matching orchestration, arrival, blocked-road point selection, and turn-event handoff. Keep active route/polyline ownership, last-segment memory, destination-radius checks, and route-bearing lookup in `NavigationRouteGeometryState`; keep deviation policy, confirmation, along-track direction evidence, and reroute-notice logging in `NavigationRouteDeviationHandler`; keep expected route-bearing lookahead in `NavigationExpectedBearingResolver`; keep compass geometry, visible-radius memory, GPS status wrapping, target progress labels, and `NavState` construction in `NavigationSessionRouteDisplayState` so route safety decisions and route presentation stay independently understandable.
...
- Keep BRouter transient-failure classification in `BRouterTransientFailureClassifier` and the default BRouter adapter in `NavigationBRouterRouteCalculator` so `NavigationRouteExecutor` stays focused on route execution, retry timing, callback handoff, and wake-lock scoping.
