...
- Keep stationary-orientation prompts conservative. Do not treat a brief low-speed GPS sample as a stop when recent filtered displacement still shows walking movement, suppress those prompts while route recalculation is in progress, and when the geomagnetic rotation-vector sample exposes a heading-accuracy estimate use it to reject ambiguous turn-yourself notifications.
