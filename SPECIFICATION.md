...
- Android Auto must reflect phone landscape compass-overlay controls in driver-safe surface form, including speed-limit display, settings, GPX export, and the configured custom button when that phone setting is enabled.
- Android Auto must reflect the fullscreen-route setting by expanding the route compass view behind the car surface while keeping navigation text, status details, and controls legible in front.
