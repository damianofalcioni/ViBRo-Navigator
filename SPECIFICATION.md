...
- Unchanged chunk selection, cached contents, street filter, and segment limit should reuse the same immutable display overlay. Heading-only redraws must reuse prepared street geometry, rotate it with the canvas, and draw the combined street path once; changes to position, geometry, radius, or drawing scale must rebuild it. Clipping must preserve segments crossing the viewport even when both endpoints are outside it
