...
- Surrounding-street extraction must classify BRouter `highway` way tags and hard-exclude non-useful inactive context types before they enter the session cache: `construction`, `proposed`, `planned`, `virtual`, `abandoned`, `disused`, `razed`, `demolished`, `dismantled`, `no`, and `bus_stop`
...
- Low speed must keep all cacheable surrounding-street types; medium speed must keep `motorway`, `motorway_link`, `trunk`, `trunk_link`, `primary`, `primary_link`, `secondary`, `secondary_link`, `tertiary`, `tertiary_link`, `unclassified`, `residential`, `service`, `road/yes`, `busway`, `rest_area`, `services`, and `raceway`; high speed must keep `motorway`, `motorway_link`, `trunk`, `trunk_link`, `primary`, `primary_link`, `secondary`, `secondary_link`, `rest_area`, `services`, and `raceway`
- `elevator` and `via_ferrata` surrounding-street types must remain cacheable but low-speed only; `raceway` must remain visible in low, medium, and high speed buckets
...
- The red target-direction cue must stay behind the semi-transparent white heading guide and distance/time labels so those foreground references remain legible when the cue is visible
...
- In portrait fullscreen route mode, the fixed white heading guide should show only the farthest distance/travel-time label pair; compact compass views and landscape fullscreen route mode should keep all three scale label pairs.
...
- The chooser-backed GPX stream file must be written to the same persistent app-managed `gpx/` folder as Auto-save GPX, using the timestamped `vibro-navigator-route-yyyymmddhhmmss.gpx` pattern with a collision suffix when needed
...
- Accepted-fix waypoints must also carry available provider, horizontal accuracy, speed, bearing and bearing-accuracy measurements in GPX extensions; those measurements describe the provider fix while exported coordinates are filtered.
...
- When the Auto-save GPX on stop setting is enabled, confirming the active navigation Stop button must save the current exportable GPX route before stopping navigation
- Automatically saved GPX files must be written under the same app-files root used for logs, in a sibling `gpx/` folder, using the filename pattern `vibro-navigator-route-yyyymmddhhmmss.gpx` with a collision suffix when needed
- The export flow must expose the saved app-private GPX file through a read-only content URI with a temporary read grant, not broad storage permissions
...
- GitHub Actions should run the APK build workflow for normal commit and pull-request CI. F-Droid readiness checks should be maintainer-operated manually, run from a pushed version tag before release, or run automatically as a required pre-submit gate before preparing a `fdroiddata` merge request. Official publication still requires F-Droid maintainer review and F-Droid-side rebuild/sign/publish steps.
