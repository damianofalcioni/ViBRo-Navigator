...
- When a reroute is applied, the compass passed-route overlay must retain already passed segments from the previous active route and append only the newly passed segment from each subsequent active route, without duplicating earlier archives. If there is a gap between the last displayed point of the archived passed segment and the first point of the newly recalculated route, the compass must draw that gap as a straight dotted red connector.
