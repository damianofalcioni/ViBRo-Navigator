...
- When a stationary orientation notification is emitted and the navigation UI is visible, the compass must show a matching red turn-to-face-route cue: a target marker just outside the compass at the target heading plus a very thin red partial arc from the current heading toward that target, leaving a visual gap before the marker so the arc does not merge with or cover the marker
...
- That calibration background must span from the outer visible distance ring to the outer border of the compass, sit behind the outer compass ticks and cardinal labels, and remain visually separate from the stationary orientation cue drawn outside the compass
...
- Missing numeric heading-accuracy data alone must not force the calibration background red; the warning should require explicit poor heading accuracy from the selected displayed heading source
- While navigation is paused, the compass must show a gray outer paused-state ring around the compass in addition to the paused-state status text and resume/play button
