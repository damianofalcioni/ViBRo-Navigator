...
- The compass must show a transient outer calibration ring around the compass when the selected displayed heading source becomes explicitly inaccurate enough to need recalibration or recovery: red while that selected source is inaccurate, green when it returns to acceptable accuracy, and the green ring must disappear automatically after about 2 seconds
- That calibration ring must follow the same selected heading source that drives the rendered compass heading, such as trusted GPS/course while moving, movement-derived course when GPS/course bearing is not trustworthy, or live heading-sensor heading when stationary or otherwise needed; it must not show a raw heading-sensor calibration warning while the compass is currently rendered from a different trusted source
- Missing numeric heading-accuracy data alone must not force the calibration ring red; the warning should require explicit poor heading accuracy from the selected displayed heading source
